...
try {
    $stmt = $pdo->query("
        SELECT
            ...
            COALESCE(SUM(
                CASE
                    WHEN i.loc IN ('INVENTORY', 'WAREHOUSE')
                    THEN i.count
                    ELSE 0
                END
            ), 0) AS adena
...
try {
    $stmt = $pdo->prepare("
        ...
        ORDER BY
            (
                SUM(
                    CASE
                        WHEN transaction_type IN ('ADD', 'REMOVE')
                        THEN amount
                        ELSE 0
                    END
                )
    ...
    $activity5m = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
try {
    ...
    $activity24h = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
details.panel summary.panel-header::after {
    content: '▼';
    font-size: 10px;
    color: #667b91;
...
details.panel[open] summary.panel-header::after {
